...
struct PathDetailView: View {
    ...
    private func exportPDF() {
        let renderer = ImageRenderer(content: PDFContent(result: result))
        renderer.scale = 2.0

        let url = FileManager.default.temporaryDirectory.appendingPathComponent("\(path.name) - Business Plan.pdf")
...
struct PDFContent: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            Group {
                pdfSection("Overview", path.overview)
                pdfSection("Startup Cost", path.startupCostRange)
                pdfSection("Time to First Dollar", path.timeToFirstDollar)
                pdfSection("AI-Proof Rating", "\(path.aiProofRating)/100")
                pdfSection("Customer Type", path.customerType)
                pdfSection("Starter Pricing", path.starterPricing)
            }
            ...
            Group {
                pdfSection("What Others Charge", path.starterPricing)
                pdfSection("Draft Email", path.draftEmail)
                pdfSection("Draft Text Message", path.draftTextMessage)
                pdfSection("Sales Intro Script", path.salesIntroScript)
                pdfSection("Social Media Post", path.socialMediaPost)
                pdfSection("Offer & Pricing", path.offerPricingSheet)

                let planSections = BusinessPlanGenerator.generate(for: result.businessPath)
                Text("One-Page Business Plan")
                    .font(.title2.bold())
                Divider()
    ...
    private func pdfSection(_ title: String, _ content: String) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(title)
                .font(.headline)
            Text(content)
